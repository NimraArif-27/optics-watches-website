...
// // 🔁 Update stock before sidebar opens (for all collections)
// document.getElementById("cartSidebar").addEventListener("show.bs.offcanvas", () => {
//   cart.forEach(item => {
//     // Find the product card on the current page with the same name
//     const card = [...document.querySelectorAll(".card")].find(c =>
//       c.querySelector(".card-title")?.innerText.trim() === item.name.trim()
//     );

//     // If found, update its stock from data attribute
//     if (card) {
//       const latestStock = parseInt(card.dataset.stock);
//       if (!isNaN(latestStock)) {
//         item.stock = latestStock;
//       }
//     }
//   });

//   // Save updated stock and re-render
//   localStorage.setItem("cart", JSON.stringify(cart));
//   renderCart();
// });
...
async function fetchBackendStock() {
  try {
    ...
    data.products.forEach(p => {
      const key = p.name; // you can include category if needed
      backendStock.set(key, p.stock);
    });

    // Update stock in cart
    cart.forEach(item => {
      const key = item.name;
      if (backendStock.has(key)) {
        item.stock = backendStock.get(key);
      }
    });
    localStorage.setItem("cart", JSON.stringify(cart));
    renderCart();
...
async function addToCart() {
  ...
  const stock = backendStock.get(name) || 0;
